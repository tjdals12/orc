...
워크플로우 실행을 멈추려면 `stop` 또는 `cancel`을 사용합니다. `stop`으로 멈춘 워크플로우 실행은 나중에 재개할 수 있지만, `cancel`로 취소한 워크플로우 실행은 영구적으로 종료되어 재개할 수 없습니다.

```bash
# 실행 중인 워크플로우를 정지하고 재개 가능한 상태로 만듭니다
orc workflow stop <run-id>

# 실행 중인 워크플로우를 중단합니다. 중단한 워크플로우는 재개할 수 없습니다
orc workflow cancel <run-id>

# 정지하거나 실패한 워크플로우를 이어서 실행합니다. 원래 입력을 그대로 쓰고, 끝나지 않은 노드부터 다시 시작합니다
orc workflow resume <run-id>
```

**주의:** Git을 사용하는 프로젝트에서 `stop`을 실행하면 커밋되지 않은 변경사항을 폐기합니다. 이런 프로젝트에서 실행하는 워크플로우는 변경사항을 만드는 각 노드가 작업을 마치기 전에 명시적으로 커밋하도록 작성하는 것을 권장합니다.
...
| `--detach`            | 백그라운드에서 실행을 시작하고 반환. 워크트리 훅 로그는 출력                                                    |
| `--json`              | 백그라운드에서 실행을 시작하고 JSON 출력. 워크트리 훅 로그는 생략. `--detach`와 같이 사용하면 `--detach`는 무시 |

### `orc workflow resume`

정지하거나 실패한 워크플로우 실행을 이어서 실행합니다. `stop_failed` 상태의 워크플로우 실행은 재개할 수 없습니다. `stop`을 다시 실행해 정지를 완료한 뒤 재개하세요.
...
| 옵션       | 설명                                        |
| ---------- | ------------------------------------------- |
| `--detach` | 백그라운드에서 실행을 재개하고 반환         |
| `--json`   | 재개된 실행과 노드를 JSON으로 출력하고 반환 |
...
워크플로우 실행을 정지합니다. 정지한 실행은 재개할 수 있습니다. Git 환경에서는 변경사항을 폐기합니다. (이미 커밋된 변경사항과 ignored 파일은 제외합니다.)
